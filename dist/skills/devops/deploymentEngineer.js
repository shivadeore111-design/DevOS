...
exports.DeploymentEngineer = void 0;
...
class DeploymentEngineer {
    ...
    async build(projectDir, tag) {
        ...
        console.log(`[DeploymentEngineer] Docker build ${result.success ? "completed" : "failed"} in ${duration}ms`);
        return {
    ...
    async healthCheck(containerName) {
        console.log(`[DeploymentEngineer] Running health check for container ${containerName}`);
        const psResult = await this.terminalOperator.execute(`docker ps --filter "name=${containerName}" --format "{{.Names}}|{{.Status}}"`);
        if (!psResult.success) {
        ...
        const inspectResult = await this.terminalOperator.execute(`docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}{{.State.Status}}{{end}}' ${containerName}`);
    ...
    async rollback(containerName, previousTag) {
        console.log(`[DeploymentEngineer] Starting rollback for ${containerName} using image tag ${previousTag}`);
        const start = Date.now();
        const steps = [];
        ...
        steps.push(`[remove] ${[removeResult.stdout, removeResult.stderr].filter(Boolean).join("\n").trim()}`);
        const runResult = await this.terminalOperator.execute(`docker run -d --name ${containerName} ${previousTag}`, { timeout: 120000 });
        steps.push(`[run] ${[runResult.stdout, runResult.stderr].filter(Boolean).join("\n").trim()}`);
        const duration = Date.now() - start;
        const success = runResult.success;
        console.log(`[DeploymentEngineer] Rollback ${success ? "completed" : "failed"} for ${containerName} in ${duration}ms`);
        return {
            success,
            tag: previousTag,
...
exports.DeploymentEngineer = DeploymentEngineer;
