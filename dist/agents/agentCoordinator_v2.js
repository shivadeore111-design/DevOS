...
exports.initPromptEvolution = initPromptEvolution;
// ============================================================
...
const promptEvolver_1 = require("../core/promptEvolver");
...
function initPromptEvolution() {
    for (const [role, prompt] of Object.entries(BASE_PROMPTS)) {
        promptEvolver_1.PromptEvolver.init(role, prompt);
    }
}
// ── Helpers ───────────────────────────────────────────────────
...
class AgentCoordinator {
    static async spawnAgent(role, goal, context = {}) {
        const task = {
            id: (0, uuid_1.v4)(),
            role,
            goal,
        ...
        if (Object.keys(context).length > 0) {
            const depOutputs = context.dependencyOutputs ?? {};
            const compressed = await (0, contextCompressor_1.compressDependencyOutputs)(depOutputs, goal);
            if (compressed && compressed.trim()) {
                contextStr = `\n\nContext from prior agents:\n${compressed}`;
            }
        }
        // ── Get evolved prompt for this role ──────────────────────
        const systemPrompt = promptEvolver_1.PromptEvolver.getPrompt(role, BASE_PROMPTS[role]);
        try {
            const response = await (0, router_1.llmCall)(`Goal: ${goal}${contextStr}`, systemPrompt);
            ...
            observability_1.Observability.recordAgentScore({
                agentId: task.id,
                role,
            ...
            memoryStore_1.memoryStore.set(`agent_${task.id}`, {
            ...
            // Index failure
            await ragRetriever_1.RAGRetriever.indexError(role, task.error ?? "", goal);
        }
        return task;
    }
    static async reviewAgentOutput(task, output) {
        const prompt = `Review this ${task.role} agent output.
Goal: ${task.goal}
Output: ${output.slice(0, 3000)}${output.length > 3000 ? "\n...[truncated]" : ""}`;
        const reviewerPrompt = promptEvolver_1.PromptEvolver.getPrompt("reviewer", BASE_PROMPTS.reviewer);
        const response = await (0, router_1.llmCall)(prompt, reviewerPrompt);
        ...
        if (!review.pass) {
            await promptEvolver_1.PromptEvolver.recordScore(task.role, score, review.improvements, task.goal);
        }
        else {
            // Record good score too (updates rolling average)
            await promptEvolver_1.PromptEvolver.recordScore(task.role, score, [], task.goal);
        }
    ...
    static async coordinateAgents(masterGoal, availableRoles) {
...
[
  ...
  { "role": "coder", "goal": "specific goal using researcher output", "dependencyIndices": [0] }
        ...
        const response = await (0, router_1.llmCall)(planningPrompt, "You are an execution planner. Return ONLY valid JSON array — no markdown, no explanation.");
        const parsed = safeParseJSON(response.content);
    ...
    static async runAgentPipeline(goal) {
        console.log(`\n[AgentCoordinator] 🚀 Pipeline: "${goal}"\n`);
        // Init prompt evolution on each pipeline run
        initPromptEvolution();
        const roles = ["researcher", "coder", "marketer", "deployer"];
        ...
        memoryStore_1.memoryStore.set("last_pipeline_run", { goal, passed, failed, avgScore, summary, completedAt: Date.now() });
        console.log(`\n[AgentCoordinator] ✅ ${summary}\n`);
        console.log(promptEvolver_1.PromptEvolver.report());
        return { goal, tasks: executedTasks, reviews, passed, failed, summary };
    }
    static formatPipelineReport(result) {
        const lines = [
            "╔══════════════════════════════════════════╗",
