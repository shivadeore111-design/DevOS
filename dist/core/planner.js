...
const planGoal = (goal) => generatePlan(goal);
exports.planGoal = planGoal;
async function generatePlan(goal) {
    console.log(`[Planner] Generating plan for: "${goal}"`);
    const { content, provider, tokensEstimate } = await (0, router_1.llmCall)(goal, SYSTEM_PROMPT);
