...
exports.CapabilityGraph = void 0;
// ============================================================
...
const GRAPH_FILE = path_1.default.join(process.cwd(), "workspace", "memory", "capabilityGraph.json");
// ── Graph Store ───────────────────────────────────────────────
function loadGraph() {
    try {
        if (fs_1.default.existsSync(GRAPH_FILE)) {
            return JSON.parse(fs_1.default.readFileSync(GRAPH_FILE, "utf-8"));
        }
    }
    catch { /* fallback */ }
...
function saveGraph(graph) {
    const dir = path_1.default.dirname(GRAPH_FILE);
    if (!fs_1.default.existsSync(dir))
        fs_1.default.mkdirSync(dir, { recursive: true });
    const tmp = GRAPH_FILE + ".tmp";
    fs_1.default.writeFileSync(tmp, JSON.stringify(graph, null, 2));
    fs_1.default.renameSync(tmp, GRAPH_FILE);
}
// ── CapabilityGraph ───────────────────────────────────────────
...
Return JSON:
{
  ...
  "blockers": ["why it can't execute if canExecute=false"]
     ...
     */
    static register(name, description, skillFile, category = "skill") {
        const graph = loadGraph();
        graph[name] = {
            id: name,
            name,
     ...
     */
    static async suggestNextBuilds(recentGoals) {
...
exports.CapabilityGraph = CapabilityGraph;
