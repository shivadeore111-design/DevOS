...
async function racePlannerAPIs(promptText, topN = 2) {
    ...
    const callOne = async (api, ctrl) => {
        ...
        if (!r.ok)
            throw new Error(`${api.provider} ${r.status}`);
    ...
    const promises = apis.map((api, i) => callOne(api, controllers[i]).then(text => {
...
async function planWithLLM(message, history, apiKey, model, provider, memoryContext) {
    ...
    try {
        const recalled = await (0, memoryRecall_1.unifiedMemoryRecall)(message, 3);
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        ...
        try {
            ...
            if (!raw || raw.trim().length === 0) {
                ...
                try {
                    (0, router_1.markRateLimited)(curProvider);
                }
                catch { }
            }
            else {
                ...
                if (!jsonMatch) {
                    ...
                    try {
                        (0, router_1.incrementUsage)(curProvider);
                    }
                    catch { }
...
async function executePlan(plan, onStep, onPhaseChange, existingState, replanApiKey, replanModel, replanProvider) {
    ...
    async function executeSingleStep(step, stepOutputs, state, plan, workspace, onStep) {
        ...
        let toolResult = await (0, toolRegistry_1.executeTool)(step.tool, resolvedInput);
    ...
    while (_gi < groups.length) {
        ...
        if (unskipped.length === 1) {
            ...
            // —— Parallel group ———————————————————————
            livePulse_1.livePulse.act('Aiden', `Running ${unskipped.length} steps in parallel: ${unskipped.map(s => s.tool).join(', ')}`);
            const settled = await Promise.allSettled(unskipped.map(step => executeSingleStep(step, stepOutputs, state, plan, workspace, onStep)));
...
async function respondWithResults(originalMessage, plan, results, history, userName, apiKey, model, providerName, onToken) {
    ...
    const loadedSkills = skillLoader_1.skillLoader.loadAll();
