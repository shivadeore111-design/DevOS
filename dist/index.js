...
const goalParser_1 = require("./core/goalParser");
...
const strategicPlanner_1 = require("./core/strategicPlanner");
const growthEngine_1 = require("./growth/growthEngine");
...
const skillSanitizer_1 = require("./executor/skillSanitizer");
const skillWarmup_1 = require("./executor/skillWarmup");
...
const productManager_1 = require("./devos/product/productManager");
const productGenerator_1 = require("./devos/product/productGenerator");
const deploymentOrchestrator_1 = require("./devos/product/deploymentOrchestrator");
...
const permissions_1 = require("./api/middleware/permissions");
...
const installer_1 = require("./cli/installer");
...
const teachMode_1 = require("./personal/teachMode");
...
taskStore_1.taskStore.load();
memoryStore_1.memoryStore.load();
vectorMemory_1.vectorMemory.load();
// Sanitize any LLM-generated skill files on every startup
skillSanitizer_1.skillSanitizer.sanitizeDirectory("skills/generated").then(n => {
    if (n > 0)
        console.log(`[Boot] Sanitized ${n} generated skill file(s)`);
}).catch(() => { });
// ── Ollama health check ────────────────────────────────────────
...
const goalArgs = rawArgs.filter((a) => !a.startsWith("--"));
const goal = goalArgs.join(" ").trim();
if (cleanMode) {
    console.log("🧹 Cleaning sandbox...");
    if (fs_1.default.existsSync(workspace)) {
        for (const f of fs_1.default.readdirSync(workspace)) {
            fs_1.default.rmSync(path_1.default.join(workspace, f), { recursive: true, force: true });
        }
    }
    console.log("🧹 Done.");
}
...
async function runDoctor() {
    ...
    // Ollama running
    try {
        await axios_1.default.get(baseUrl, { timeout: 3000 });
    ...
    // Model available
    try {
        const res = await axios_1.default.get(`${baseUrl}/api/tags`, { timeout: 5000 });
    ...
    // Workspace + .env
    const wsExists = fs_1.default.existsSync(path_1.default.join(process.cwd(), "workspace"));
    ...
    for (const f of newFiles) {
        const exists = fs_1.default.existsSync(path_1.default.join(process.cwd(), f));
...
async function runTest() {
    ...
    try {
        const plan = await strategicPlanner_1.StrategicPlanner.createStrategicPlan({
    ...
    try {
        const icp = await growthEngine_1.GrowthEngine.generateICP("A simple todo app for busy professionals");
    ...
    // Save report
    const logDir = path_1.default.join(process.cwd(), "workspace", "logs");
    fs_1.default.mkdirSync(logDir, { recursive: true });
    fs_1.default.writeFileSync(path_1.default.join(logDir, "test-report.json"), JSON.stringify({ timestamp: new Date().toISOString(), passed, total, runtime, results }, null, 2));
...
async function handleCLI() {
    ...
    console.log("");
    const engine = new engine_1.DevOSEngine(workspace, dryRun);
    switch (command) {
        // ── devos run ────────────────────────────────────────────
        case "run": {
            ...
            if (rawArgs[0]?.toLowerCase() === 'workflow') {
                ...
                if (!wfName) {
                    console.error('❌ Usage: ts-node index.ts run workflow "<name>"');
                    break;
                }
                console.log(`\n▶️  Running workflow: "${wfName}"\n`);
                const wfOutput = await teachMode_1.teachMode.runWorkflow(wfName);
            ...
            if (goal === "test-gap") {
                ...
                const parsed = (0, goalParser_1.parseGoal)(goal);
                const memHint = executionMemory_1.executionMemory.lookup(parsed);
                let extraCtx;
                ...
                plan = await (0, planner_v2_1.generatePlan)(goal, extraCtx);
            }
            console.log("\n📦 Plan:");
            console.log(JSON.stringify(plan, null, 2));
            const runner = new runner_1.Runner({ agentId: "cli-agent", engine });
            const task = await runner.runOnce(goal, plan);
            if (task.status === "completed") {
                console.log("\n🔍 Verifying result...");
                const verification = await (0, verifier_1.verifyTask)(task);
                ...
                memoryStore_1.memoryStore.set(`task:${task.id}`, {
                    goal: task.goal, result: task.result, verification, completedAt: task.completedAt,
                }, ["task", "completed"]);
            }
            await ragRetriever_1.RAGRetriever.indexTask({
                id: task.id,
                goal: task.goal,
                status: task.status,
                result: task.result,
            ...
            if (task.status === "completed") {
                await taskPatterns_1.TaskPatternMemory.save(task.goal, plan);
            }
            observability_1.Observability.recordTask({
                taskId: task.id,
                goal: task.goal,
                status: task.status,
                durationMs: task.completedAt
        ...
        case "daemon": {
            ...
            for (let i = 0; i < agentCount; i++) {
                const runner = new runner_1.Runner({ agentId: `daemon-agent-${i + 1}`, engine, pollIntervalMs: pollMs });
                runner.startDaemon();
            }
        ...
        // ── devos status ──────────────────────────────────────────
        case "status": {
            const all = taskStore_1.taskStore.getAll();
            if (!all.length) {
        ...
        case "enqueue": {
            ...
            await assertOllamaReady();
            const plan = await (0, planner_v2_1.generatePlan)(goal);
            const task = taskQueue_1.taskQueue.create({ goal, plan });
        ...
        case "plan": {
            ...
            console.log(strategicPlanner_1.StrategicPlanner.formatPlanReport(plan));
            const savedPath = strategicPlanner_1.StrategicPlanner.savePlan(plan);
        ...
        case "grow": {
            ...
            const icp = await growthEngine_1.GrowthEngine.generateICP(goal);
            console.log(`✅ ICP generated — Segment: ${icp.segment}`);
            const landing = await growthEngine_1.GrowthEngine.generateLandingPageCopy(icp, goal);
            console.log(`✅ Landing page copy — Headline: "${landing.headline}"`);
            await growthEngine_1.GrowthEngine.generateEmailSequence("Welcome", "user_signup", icp);
            console.log("✅ Email sequence generated");
            await growthEngine_1.GrowthEngine.generateContentCalendar(4, icp);
        ...
        case "agent": {
            ...
            const knownRoles = ["ceo", "engineer", "researcher", "operator"];
            if (knownRoles.includes(agentArg)) {
                // ── Sprint 14: devos agent <role> — show detail
                const agentDetail = agentRegistry_1.agentRegistry.get(agentArg);
                if (!agentDetail) {
                ...
                const recentMsgs = agentMessenger_1.agentMessenger.getRecent(50).filter(m => m.fromAgent === agentArg || m.toAgent === agentArg || m.toAgent === "all").slice(-10);
            ...
            const result = await agentCoordinator_v2_1.AgentCoordinator.runAgentPipeline(goal);
            const agentReport = agentCoordinator_v2_1.AgentCoordinator.formatPipelineReport(result);
        ...
        // ── devos serve ───────────────────────────────────────────
        case "serve": {
            await skillWarmup_1.skillWarmup.preloadOnServe();
            const { dashboardServer } = await Promise.resolve().then(() => __importStar(require("./dashboard/server")));
            await dashboardServer.start();
            (0, triggers_1.startAllTriggers)();
            pilotScheduler_1.pilotScheduler.start();
            (0, server_1.startApiServer)(api_json_1.default.port);
            ...
            console.log(`   REST API            at http://localhost:${api_json_1.default.port}`);
            console.log(`   API Docs            at http://localhost:${api_json_1.default.port}/api/docs`);
            console.log("   Press Ctrl+C to stop");
            const onServeSig = () => {
                dashboardServer.stop();
                (0, triggers_1.stopAllTriggers)();
                pilotScheduler_1.pilotScheduler.stop();
                process.exit(0);
            };
        ...
        // ── devos dashboard ───────────────────────────────────────
        case "dashboard": {
            const { execSync: execD } = require('child_process');
            const dashPath = path_1.default.join(process.cwd(), 'dashboard-next');
        ...
        case "capabilities": {
            if (!goal) {
                console.log(capabilityGraph_1.CapabilityGraph.report());
                break;
            }
            await assertOllamaReady();
            const analysis = await capabilityGraph_1.CapabilityGraph.analyzeGoal(goal);
            console.log("\n🔍 Capability Analysis:");
            ...
            if (analysis.missing.length > 0) {
                ...
                console.log("\n  Skills to build:");
                analysis.gaps.forEach(g => console.log(`    • ${g.required}: ${g.buildHint ?? "build a skill for this"}`));
            }
            break;
        }
        // ── devos company ─────────────────────────────────────────
        ...
        case "memory": {
            ...
            const { executionMemory: em } = await Promise.resolve().then(() => __importStar(require("./memory/executionMemory")));
            if (subCmd === "prune") {
                const { memoryAging } = await Promise.resolve().then(() => __importStar(require("./memory/memoryAging")));
        ...
        case "research": {
            ...
            console.log(`\n🔍 Researching: "${goal}"\n`);
            const parsed = (0, goalParser_1.parseGoal)(goal);
            const report = await researchEngine_1.researchEngine.research(goal, parsed);
            console.log("\n" + "═".repeat(60));
        ...
        // ── devos sessions ───────────────────────────────────────
        case "sessions": {
            const { sessionManager } = await Promise.resolve().then(() => __importStar(require("./core/sessionManager")));
        ...
        case "stop": {
            ...
            if (!taskId) {
                ...
                if (teachMode_1.teachMode.isRecording()) {
                    const wf = teachMode_1.teachMode.stopRecording();
            ...
            const { emergencyStop } = await Promise.resolve().then(() => __importStar(require("./control/emergencyStop")));
        ...
        case "resume": {
            ...
            await assertOllamaReady();
            const { stateSnapshot, graphFromSnapshot } = await Promise.resolve().then(() => __importStar(require("./devos/runtime/stateSnapshot")));
            const { createGraphExecutor } = await Promise.resolve().then(() => __importStar(require("./core/graphExecutor")));
            ...
            const resumeEngine = new engine_1.DevOSEngine(workspace, dryRun);
            const graphExecutor = createGraphExecutor((action, wp) => resumeEngine.executeOne(action, wp));
        ...
        // ── devos skills ──────────────────────────────────────────
        case "skills": {
            const all = skillIndex_1.skillIndex.getAll();
            if (!all.length) {
            ...
            const byTier = {};
            for (const s of all) {
                (byTier[_a = s.tier] ?? (byTier[_a] = [])).push(s);
            }
        ...
        case "cron": {
            ...
            if (sub === "list" || !sub) {
                const jobs = triggers_1.cronTrigger.list();
                if (!jobs.length) {
            ...
            if (sub === "add") {
                ...
                const id = triggers_1.cronTrigger.add({ schedule, goal: cronGoal, enabled: true });
                console.log(`\n✅ Cron job added: ${id}`);
            ...
            if (sub === "remove") {
                ...
                triggers_1.cronTrigger.remove(id);
                console.log(`\n✅ Cron job removed: ${id}\n`);
                break;
            }
            if (sub === "enable") {
                triggers_1.cronTrigger.enable(goalArgs[1] ?? "");
                console.log(`✅ Enabled: ${goalArgs[1]}`);
                break;
            }
            if (sub === "disable") {
                triggers_1.cronTrigger.disable(goalArgs[1] ?? "");
        ...
        case "webhook": {
            const sub = goalArgs[0];
            if (sub === "list" || !sub) {
                const hooks = triggers_1.webhookTrigger.list();
                if (!hooks.length) {
            ...
            if (sub === "add") {
                ...
                triggers_1.webhookTrigger.register(webhookPath, hookGoal);
        ...
        case "knowledge": {
            ...
            if (sub === "ingest") {
                ...
                if (kTarget.startsWith("http://") || kTarget.startsWith("https://")) {
                    const id = await ingestionEngine_1.ingestionEngine.ingestUrl(kTarget);
                    const entry = knowledgeStore_1.knowledgeStore.get(id);
                    if (entry) {
                    ...
                    const id = await ingestionEngine_1.ingestionEngine.ingest(kTarget);
                    const entry = knowledgeStore_1.knowledgeStore.get(id);
                    if (entry) {
            ...
            if (sub === "query") {
                ...
                const result = await knowledgeQuery_1.knowledgeQuery.query(kTarget);
            ...
            {
                const all = knowledgeStore_1.knowledgeStore.list();
                if (!all.length) {
        ...
        case "github": {
            ...
            if (sub === "issues" || !sub) {
                let cfg = {};
                try {
                    cfg = JSON.parse(fs_1.default.readFileSync("config/integrations.json", "utf-8"));
                }
                catch { }
                const repo = goalArgs[1] ?? cfg?.github?.defaultRepo ?? "";
                const state = goalArgs[2] ?? "open";
                if (!repo) {
                ...
                try {
                    const issues = await github_1.github.listIssues(repo, state);
                    if (!issues.length) {
        ...
        case "pilot": {
            ...
            // devos pilot list
            if (sub === "list" || !sub) {
                const all = pilotRegistry_1.pilotRegistry.list();
                if (!all.length) {
                ...
                for (const p of all) {
                    ...
                    const lastRun = pilotExecutor_1.pilotExecutor.getLastRun(p.id);
            ...
            if (sub === "run") {
                ...
                const run = await pilotExecutor_1.pilotExecutor.run(pilotId);
            ...
            if (sub === "enable") {
                if (!pilotId) {
                    console.error("❌ Usage: ts-node index.ts pilot enable <id>");
                    process.exit(1);
                }
                pilotRegistry_1.pilotRegistry.enable(pilotId);
            ...
            if (sub === "disable") {
                if (!pilotId) {
                    console.error("❌ Usage: ts-node index.ts pilot disable <id>");
                    process.exit(1);
                }
                pilotRegistry_1.pilotRegistry.disable(pilotId);
            ...
            if (sub === "history") {
                if (!pilotId) {
                    console.error("❌ Usage: ts-node index.ts pilot history <id>");
                    process.exit(1);
                }
                const history = pilotExecutor_1.pilotExecutor.getHistory(pilotId).slice(0, 5);
                if (!history.length) {
        ...
        // ── devos blueprints ──────────────────────────────────────
        case "blueprints": {
            const bps = blueprintRegistry_1.blueprintRegistry.list();
            if (!bps.length) {
        ...
        // ── devos products ────────────────────────────────────────
        case "products": {
            const builds = productManager_1.productManager.list();
            if (!builds.length) {
        ...
        case "build": {
            ...
            // Match blueprint
            const { parseGoal: pg } = await Promise.resolve().then(() => __importStar(require("./core/goalParser")));
            const parsed = pg(goal);
            const bp = blueprintRegistry_1.blueprintRegistry.match({ ...parsed, goal });
            if (!bp) {
            ...
            const buildDir = path_1.default.join(process.cwd(), "workspace", "builds", bp.id + "_" + Date.now());
            fs_1.default.mkdirSync(buildDir, { recursive: true });
            // Generate
            const build = await productGenerator_1.productGenerator.generate(goal, bp.id, buildDir);
            if (build.status === "completed") {
                ...
                // Deploy locally
                console.log("\n🚀 Deploying locally...");
                const deploy = await deploymentOrchestrator_1.deploymentOrchestrator.deploy(build, "local");
                if (deploy.success) {
        ...
        case "goal": {
            ...
            if (sub === "status") {
                ...
                const status = await goalEngine_1.goalEngine.getStatus(goalId);
                if (!status.goal) {
                ...
                if (g.completedAt)
                    console.log(`   Completed:   ${new Date(g.completedAt).toLocaleString()}`);
            ...
            const finalGoal = await goalEngine_1.goalEngine.run(titleArg, descArg);
        ...
        // ── devos goals ───────────────────────────────────────────
        case "goals": {
            const all = await goalEngine_1.goalEngine.list();
            if (!all.length) {
        ...
        case "agents": {
            ...
            if (rawArgs[0]?.toLowerCase() === 'personal') {
                const bgAgents = backgroundAgents_1.backgroundAgents.listAgents();
            ...
            const all = agentRegistry_1.agentRegistry.list();
        ...
        case "coordinate": {
            ...
            const coordGoal = goalStore_1.goalStore.createGoal(titleArg, descArg);
            console.log(`[Coordinate] 🎯 Goal created: ${coordGoal.id}`);
            const { goalPlanner } = await Promise.resolve().then(() => __importStar(require("./goals/goalPlanner")));
            await goalPlanner.plan(coordGoal.id);
            await coordinationLoop_1.coordinationLoop.start(coordGoal.id);
            const finalCoordGoal = goalStore_1.goalStore.getGoal(coordGoal.id);
        ...
        case "api": {
            ...
            if (apiSub === "keygen") {
                ...
                const key = (0, permissions_1.generateApiKey)(role, label);
                console.log(`\n✅ API Key generated:`);
                console.log(`   Key:   ${key}`);
            ...
            console.log(`
╔══════════════════════════════════════════════════════════════╗
║              DevOS REST API  (port ${api_json_1.default.port})               ║
╚══════════════════════════════════════════════════════════════╝

Start the API:  ts-node index.ts serve
Docs:           http://localhost:${api_json_1.default.port}/api/docs
...
`);
        ...
        case "mission": {
            ...
            const { missionState: ms } = await Promise.resolve().then(() => __importStar(require('./coordination/missionState')));
            const { missionTodo: mt } = await Promise.resolve().then(() => __importStar(require('./coordination/missionTodo')));
            const { eventBus: eb } = await Promise.resolve().then(() => __importStar(require('./core/eventBus')));
            const sub = goalArgs[0];
            // devos mission status <id>
            if (sub === 'status') {
                const mId = goalArgs[1];
                if (!mId) {
                    console.error('❌ Usage: ts-node index.ts mission status <id>');
                    process.exit(1);
                }
                const m = ms.loadMission(mId);
                if (!m) {
                    console.error(`❌ Mission not found: ${mId}`);
                    process.exit(1);
                }
                console.log(`\n🚀 Mission: ${m.goal}`);
                console.log(`   ID:      ${m.id}`);
                ...
                if (m.completedAt)
                    console.log(`   Ended:   ${new Date(m.completedAt).toLocaleString()}`);
            ...
            if (sub === 'pause') {
                const mId = goalArgs[1];
                if (!mId) {
                    console.error('❌ Usage: ts-node index.ts mission pause <id>');
                    process.exit(1);
                }
            ...
            if (sub === 'resume') {
                const mId = goalArgs[1];
                if (!mId) {
                    console.error('❌ Usage: ts-node index.ts mission resume <id>');
                    process.exit(1);
                }
                await assertOllamaReady();
            ...
            if (sub === 'cancel') {
                const mId = goalArgs[1];
                if (!mId) {
                    console.error('❌ Usage: ts-node index.ts mission cancel <id>');
                    process.exit(1);
                }
        ...
        case "missions": {
            const { missionState: ms2 } = await Promise.resolve().then(() => __importStar(require('./coordination/missionState')));
            ...
            if (message) {
                ...
                const prompt = () => {
                    rl.question('You: ', async (input) => {
                        ...
                        for await (const chunk of dialogueEngine_1.dialogueEngine.chat(text)) {
                            process.stdout.write(chunk);
                        }
                        process.stdout.write('\n\n');
                        prompt();
        ...
        case "profile": {
            ...
            if (sub === 'reset') {
                userProfile_1.userProfile.reset();
            ...
            if (sub === 'pilots') {
                ...
                if (onOff === 'on' || onOff === 'off') {
                    const p = userProfile_1.userProfile.patch({ pilotsEnabled: onOff === 'on' });
        ...
        case "personal": {
            ...
            let envContent = fs_1.default.existsSync(envFile) ? fs_1.default.readFileSync(envFile, 'utf-8') : '';
        ...
        case "builder": {
            const envFile2 = path_1.default.join(process.cwd(), '.env');
            let envContent2 = fs_1.default.existsSync(envFile2) ? fs_1.default.readFileSync(envFile2, 'utf-8') : '';
            ...
            fs_1.default.writeFileSync(envFile2, envContent2);
        ...
        case "briefing": {
            ...
            const text = await morningBriefing_1.morningBriefing.generate();
            console.log(text);
            console.log('');
            break;
        }
        ...
        case "teach": {
            ...
            teachMode_1.teachMode.startRecording(workflowName);
            break;
        }
...
When running devos serve, DevOS will:
  ...
  • Route dangerous action approvals through inline buttons
        `.trim());
            ...
            if (sub === 'status') {
                const cfgPath = path_1.default.join(process.cwd(), 'config/integrations.json');
                const cfg = JSON.parse(fs_1.default.readFileSync(cfgPath, 'utf-8')).telegram || {};
            ...
            if (sub === 'test') {
                ...
                const { telegramBot: tb } = await Promise.resolve().then(() => __importStar(require('./integrations/telegram/telegramBot')));
                await tb.start();
        ...
        case "sandbox": {
            const sub = rawArgs[0]?.toLowerCase();
            const { sandboxManager } = await Promise.resolve().then(() => __importStar(require('./sandbox/sandboxManager')));
            if (sub === 'status' || !sub) {
        ...
        case "ui": {
            ...
            const uiDir = path_1.default.join(process.cwd(), "ui");
