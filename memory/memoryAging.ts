...
// memory/memoryAging.ts — Age, boost, and prune execution memory entries

import { ExecutionMemory, ExecutionMemoryEntry } from "./executionMemory"

const MS_PER_DAY = 86_400_000

export class MemoryAging {

  // ── Age entries — returns modified entries with pruning flags ─

  age(entries: ExecutionMemoryEntry[]): ExecutionMemoryEntry[] {
    const now = Date.now()
    const result: ExecutionMemoryEntry[] = []
  ...
  shouldPrune(entry: ExecutionMemoryEntry): boolean {
    const now     = Date.now()
    const ageMs   = now - new Date(entry.timestamp).getTime()
    const ageDays = ageMs / MS_PER_DAY

    // Old low-performers
    if (ageDays > 30 && entry.successRate < 0.3) return true

    // Never used after 7 days
    if (ageDays > 7 && entry.useCount === 0) return true
  ...
  runAging(memory: ExecutionMemory): void {
...
export const memoryAging = new MemoryAging()
