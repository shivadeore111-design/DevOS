...
export type CostFindingType =
  | "unnecessary_api_call"
  | "unoptimized_query"
  | "redundant_compute"
  | "missing_cache"
  | "n_plus_one"
  | "large_payload"
  | "polling_instead_of_webhook"
  | "unused_dependency";
...
export interface CostFinding {
  ...
  line?:             number;
  description:       string;
  estimatedSavings:  string;   // e.g. "$12/mo", "40% reduction", "2s per request"
...
export interface CostReport {
  ...
  quickWins:            CostFinding[];
  generatedAt:          string;
}

// ── Skill ─────────────────────────────────────────────────────

export class CostOptimizer implements Skill {
  readonly name        = "cost_optimizer";
  readonly description = "Scans a project for unnecessary API calls, unoptimized queries, and redundant compute — returns findings with estimated savings";

  async execute(args: { projectDir: string }): Promise<CostReport> {
    return this.analyze(args.projectDir);
  }

  async analyze(projectDir: string): Promise<CostReport> {
    ...
    const codeSnippets = tsFiles
      ...
      .map(f => {
        ...
        const src  = fs.readFileSync(f, "utf-8").slice(0, 800);
        return `// ${rel}\n${src}`;
      })
      .join("\n\n---\n\n");
...
Return JSON:
{
  ...
  "totalEstimatedSavings": "$50/mo"
    ...
    const llmResult = await llmCallJSON<{
      findings: CostFinding[];
      summary: string;
      totalEstimatedSavings: string;
  ...
  private runStaticScan(files: string[], baseDir: string): CostFinding[] {
  ...
  private collectSourceFiles(dir: string, exts = [".ts", ".js"]): string[] {
  ...
  private deduplicateFindings(findings: CostFinding[]): CostFinding[] {
