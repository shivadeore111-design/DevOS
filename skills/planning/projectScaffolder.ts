...
export type ProjectType = "express-api" | "nextjs" | "fastapi" | "react" | "cli-tool";
...
export class ProjectScaffolder implements Skill {
  readonly name        = "project_scaffolder";
  readonly description = "Creates full project directory structures with boilerplate for express-api, nextjs, fastapi, react, cli-tool";

  async execute(args: { projectType: string; name: string; outputDir: string }): Promise<ScaffoldResult> {
    return this.scaffold(args.projectType as ProjectType, args.name, args.outputDir);
  }

  async scaffold(projectType: ProjectType, name: string, outputDir: string): Promise<ScaffoldResult> {
  ...
  private async scaffoldExpressAPI(
    name: string, dir: string,
    terminal: TerminalOperator,
    files: string[], cmds: string[]
  ): Promise<void> {
    const write = (rel: string, content: string) => this.writeFile(dir, rel, content, files);
    ...
    if (result.exitCode !== 0) console.warn("[ProjectScaffolder] npm install warning:", result.stderr);
  }
  ...
  private async scaffoldFastAPI(
    name: string, dir: string,
    _terminal: TerminalOperator,
    files: string[], cmds: string[]
  ): Promise<void> {
    const write = (rel: string, content: string) => this.writeFile(dir, rel, content, files);
  ...
  private writeFile(baseDir: string, relative: string, content: string, tracker: string[]): void {
