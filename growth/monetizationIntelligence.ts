...
import { ICPProfile } from './growthEngine';
...
export class MonetizationIntelligence {
   ...
   */
  static async suggestPricingStrategy(
...
[
  {
    ...
    const tiers: PricingTier[] = Array.isArray(parsed)
      ? parsed.map((t: any) => ({
          name: t.name || 'Plan',
   ...
   */
  static detectConversionIssues(metrics: FunnelMetrics): FunnelAnalysis {
    ...
    const priorityFix =
      rates.visitorToSignup < BENCHMARKS.visitorToSignup.warn
    ...
    const result: FunnelAnalysis = { rates, benchmarks, dropoffs, suggestions, priorityFix };
...
[
  {
    ...
    const upsells: UpsellOpportunity[] = Array.isArray(parsed)
      ? parsed.map((u: any) => ({
   ...
   */
  static async suggestRetentionImprovements(
    churnReasons: string[]
  ): Promise<RetentionFix[]> {
...
[
  {
    ...
    const fixes: RetentionFix[] = Array.isArray(parsed)
      ? parsed.map((f: any) => ({
   ...
   */
  static async suggestChurnFixes(
...
[
  {
    ...
    const interventions: ChurnIntervention[] = Array.isArray(parsed)
      ? parsed.map((c: any) => ({
          pattern: c.pattern || '',
   ...
   */
  static async identifyExpansionRevenue(
    currentCustomers: Array<Record<string, any>>
...
[
  {
    ...
    const opportunities: ExpansionOpportunity[] = Array.isArray(parsed)
      ? parsed.map((o: any) => ({
   ...
   */
  static async generateMRROptimizationPlan(
    currentMRR: number,
    targetMRR: number
  ): Promise<MRRPlan> {
...
Return JSON:
{
  ...
  "steps": [
    ...
    const steps: MRRStep[] = Array.isArray(parsed?.steps)
  ...
  static formatRevenueReport(
    ...
    lines.push(
      '',
      '── FUNNEL ANALYSIS ─────────────────────────',
      `  Visitor→Signup:  ${funnel.rates.visitorToSignup}%`,
      `  Signup→Trial:    ${funnel.rates.signupToTrial}%`,
      `  Trial→Paid:      ${funnel.rates.trialToPaid}%`,
      `  Overall:         ${funnel.rates.overallConversion}%`,
      '',
      `  🎯 Priority: ${funnel.priorityFix}`,
    );
    ...
    lines.push(
      '',
      '── MRR GROWTH PLAN ─────────────────────────',
      `  Current: $${mrrPlan.currentMRR}  →  Target: $${mrrPlan.targetMRR}  (Gap: $${mrrPlan.gap})`,
      `  ${mrrPlan.summary}`,
      '',
      '  Steps:',
    );
