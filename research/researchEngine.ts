...
import { searchRouter }         from "../web/searchRouter";
...
export interface ResearchReport {
  ...
  rawReport:  string;
  durationMs: number;
  synthetic?: boolean;   // true when results came from LLM fallback only
}
...
export class ResearchEngine {
  ...
  async research(goal: string, parsedGoal?: ParsedGoal): Promise<ResearchReport> {
    ...
    const routerPromises = queries.map(q => searchRouter.search(q.query));
    ...
    routerResults.forEach((r, i) => {
      ...
      if (r.method === "llm") anySynthetic = true;
    });
