...
export async function compressContext(
  ...
  const prompt =
    `You are a context filter. Extract ONLY the information relevant to this goal:
"${targetGoal}"
...
- If nothing is relevant, return the single word: EMPTY`;
  ...
  try {
    const compressed = await callOllama(prompt, undefined);
    if (!compressed || compressed.trim().toUpperCase() === "EMPTY") return "";
 ...
 */
export async function compressDependencyOutputs(
...
export async function scoreRelevance(
  ...
  if (!context || context.trim().length < 20) return 0;

  const prompt =
    `Rate how relevant this context is to answering the goal below.
Goal: "${goal}"
...
Return ONLY a number between 0 and 10. No explanation. Just the number.`;
  ...
  try {
    const raw = await callOllama(prompt, undefined);
