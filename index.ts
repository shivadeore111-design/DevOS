...
When running devos serve, DevOS will:
  ...
  • Route dangerous action approvals through inline buttons
    ...
    // ── devos setup ───────────────────────────────────────────
    case 'setup': {
      const { runSetupWizard } = await import('./core/setupWizard')
    ...
    case 'setup:reset': {
      ...
      console.log('[Setup] Reset. Run devos setup to reconfigure.')
      const { runSetupWizard: wizard } = await import('./core/setupWizard')
      await wizard()
      break
    }
    ...
    // ── devos hardware ─────────────────────────────────────────
    case 'hardware': {
      const { detectHardware } = await import('./core/hardwareDetector')
