...
export interface BusinessGoal {
  ...
  /** 'bootstrap' | 'funded' | 'enterprise' or a number */
  budgetConstraint: string | number;
}
...
export class StrategicPlanner {
   ...
   */
  static async createStrategicPlan(goal: BusinessGoal): Promise<StrategicPlan> {
...
Return this exact JSON structure:
{
  ...
  "escalationTriggers": ["When to ask human for help"]
    ...
    let recommendedStack: Record<string, string> = {};
    if (Array.isArray(parsed?.recommendedStack)) {
      parsed.recommendedStack.forEach((s: string, i: number) => {
    ...
    const architectureDecision: ArchitectureDecisionStatus =
      ['approved', 'review_recommended', 'blocked'].includes(parsed?.architectureDecision)
        ? parsed.architectureDecision
        : 'review_recommended';

    const escalationTriggers: string[] = Array.isArray(parsed?.escalationTriggers)
   ...
   */
  static detectOverengineering(milestones: Milestone[]): string[] {
    const flags: string[] = [];
   ...
   */
  static detectRisks(milestones: Milestone[]): StrategicRisk[] {
    const joined = JSON.stringify(milestones).toLowerCase();
    return RISK_CHECKS
   ...
   */
  static evaluateArchitectureDecision(proposal: string): ArchitectureDecisionStatus {
   ...
   */
  static savePlan(plan: StrategicPlan): string {
    const dir = path.join(process.cwd(), 'workspace', 'plans');
    fs.mkdirSync(dir, { recursive: true });
    const filename = `plan-${Date.now()}.md`;
    const filepath = path.join(dir, filename);
    fs.writeFileSync(filepath, StrategicPlanner.formatPlanReport(plan), 'utf-8');
   ...
   */
  static formatPlanReport(plan: StrategicPlan): string {
    const lines: string[] = [
      '╔══════════════════════════════════════════╗',
