...
// Load react-markdown client-only (ESM-only package — ssr:false avoids hydration mismatch)
...
export default function Home() {
  // ── Persisted messages (display) ─────────────────────────────
  const [messages, setMessages] = useState<Message[]>(() => {
    if (typeof window === 'undefined') return []
    try {
      const saved = localStorage.getItem('devos_chat_history')
      return saved ? JSON.parse(saved) : []
    } catch { return [] }
  })

  // ── API conversation history (sent to Ollama) ─────────────────
  const [history, setHistory] = useState<HistoryEntry[]>(() => {
    if (typeof window === 'undefined') return []
    try {
      return JSON.parse(localStorage.getItem('devos_api_history') || '[]')
    } catch { return [] }
  })
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return
    try {
      localStorage.setItem('devos_chat_history', JSON.stringify(messages.slice(-50)))
    } catch {}
  }, [messages])
  ...
  const send = async () => {
    ...
    const newHistory: HistoryEntry[] = [...history, { role: 'user', content: msg }]
    ...
    try {
      const r = await fetch('http://localhost:4200/api/chat', {
        ...
        body: JSON.stringify({ message: msg, mode: searchMode, history: newHistory.slice(-10) })
      })
      const data = await r.json()
      const reply: string = data.reply || 'Done.'
      setMessages(prev => [...prev, { role: 'devos', content: reply, time: 'just now' }])
      const updatedHistory: HistoryEntry[] = [...newHistory, { role: 'assistant', content: reply }]
      setHistory(updatedHistory)
      if (typeof window !== 'undefined') {
        try { localStorage.setItem('devos_api_history', JSON.stringify(updatedHistory.slice(-20))) } catch {}
      }
    } catch {
  ...
  const clearChat = () => {
    ...
    if (typeof window !== 'undefined') {
      try {
        localStorage.removeItem('devos_chat_history')
        localStorage.removeItem('devos_api_history')
      } catch {}
    }
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ width: '100%', maxWidth: '680px' }}>
          ...
          {/* MESSAGES */}
          <div style={{ paddingTop: messages.length > 0 ? '32px' : '0', display: 'flex',
            ...
            {messages.map((msg, i) => (
              <div key={i} className="msg-in" style={{
                ...
                <div
                  className={msg.role === 'devos' ? 'msg-bubble' : undefined}
                  style={{
                    maxWidth: '80%',
                    padding: '12px 16px',
            ...
            {/* Spacer so fixed input doesn't cover last message */}
      ...
      <div style={{
        ...
        <div style={{ maxWidth: '680px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{
            ...
            {/* Mode selector */}
            <div style={{ display: 'flex', gap: '4px', marginTop: '10px', marginBottom: '8px' }}>
